...
Invoke-Checked $CondaExe run -n $EnvName python -m streamlit run streamlit_app.py --server.address 127.0.0.1 --server.port 8791
```
...
```powershell
$env:STREAMLIT_BROWSER_GATHER_USAGE_STATS = "false"
conda run -n local_customgui_windows python -m streamlit run streamlit_app.py --server.address 127.0.0.1 --server.port 8791
```
