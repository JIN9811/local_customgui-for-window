...
if (Get-Command winget -ErrorAction SilentlyContinue) {
  ...
  Write-Warning "winget이 없어 Python/Ollama 자동 설치를 건너뜁니다. Python 3.11을 설치한 뒤 다시 실행하세요."
}

$Python311 = $null
try {
  $Python311 = (py -3.11 -c "import sys; print(sys.executable)") | Select-Object -First 1
} catch {}
if (-not $Python311) {
  $Candidates = @(
...
if (-not $Python311) {
  throw "Python 3.11을 찾지 못했습니다. 새 PowerShell을 열어 같은 명령을 다시 실행하거나 Python 3.11을 설치하세요."
}

& $Python311 -m venv .venv
$ProjectPython = Join-Path (Get-Location) ".venv\Scripts\python.exe"
& $ProjectPython -m pip install -U pip
...
& $ProjectPython -m streamlit run streamlit_app.py --server.address 127.0.0.1 --server.port 8791
```
...
Notebook parity가 꼭 필요하면 Python 3.11 conda 환경을 별도로 만든 뒤 학습 엔진을 `pycaret`으로 바꾼다.

```powershell
conda create -y -n local_customgui_pycaret python=3.11 pip
conda run -n local_customgui_pycaret python -m pip install -r requirements-pycaret.txt
...
.\.venv\Scripts\python.exe -m hd_serving.inference --task regression --model latest --input data\raw\Reg_extracted.xlsx --output predictions_regression.xlsx
```
...
```powershell
.\.venv\Scripts\python.exe app.py --host 127.0.0.1 --port 8790 --open
```
